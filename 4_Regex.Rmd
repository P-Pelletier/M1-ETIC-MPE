...
<style>
div.r pre { background-color: #FFF3F3; }
</style>
...
- In R, the use of % is possible for formatting strings. We can use the sprintf function to print the formatted string, but it can also be stored for later use. We can use the cat function to print the string containing escape characters, or we can use writeLines as shown in the 'get_info_2' function.
...
- Some functions in 're' (Python) and 'stringr'(R) that can be useful when working with regular expressions are:
 ...
 Stringr |Description
-|-:
	str_match_all | Returns a list containing all matches
	str_locate_all |Returns a Match object if there is a match anywhere in the string
 str_split | 	Returns a list where the string has been split at each match
 str_replace_all |	Replaces one or many matches with a string
...
# Define user agent
user_agent <- httr::user_agent("Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:131.0) Gecko/20100101 Firefox/131.0")

# Function to get the HTML page
get_page <- function(urlpage) {
  # Avoid getting banned
  Sys.sleep(7 + runif(1, 0, 8))
